...
class Bezier:
    ...
    def calc_bezier_path_G1(self, WayPoints):
        ...
        for i in range(imax):
            for t in np.linspace(0, 1, self.samplePoints):  
                traj.append(self.bezier(t, controlPoints[0 + i * 2: 3 + i * 2]))
        path = np.array(traj)

        return path

    def WayPoints_to_ControlPoints(self, WayPoints):  
        WayPointsSample = []
        sampling = 1
        for i in range(int(len(WayPoints)/sampling)):  
            WayPointsSample.append(WayPoints[i*sampling])
        WayPointsSample.append(WayPoints[len(WayPoints)-1]) 
