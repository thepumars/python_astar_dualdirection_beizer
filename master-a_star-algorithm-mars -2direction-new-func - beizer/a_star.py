...
class Astar:
    ...
    def TotalCost(self,p1,p2):
        return (1-self.b)*(self.BaseCost(p1,point.Point(self.map.size-1,self.map.size-1))/
                           (1+self.a)+self.a*self.BaseCost(p1,p2)/(1+self.a))+self.b*self.Gcost(p1)
    ...
    def BuildPath(self, p1,p2, ax, plt, start_time):
        ...
        for p in path:
            ...
            if p.x == 0 and p.y == 0:
                way = np.array([[0,0]])
                continue
            way = np.append(way,[[p.x,p.y]],axis = 0)
            
        print(way)
        b = Bezier.Bezier(100)
        return b.calc_bezier_path_G1(way)
    def RunAndSaveImage(self,ax,plt):
        ...
        while self.open_set1 and self.open_set2:
            ...
            if self.target_set:
                res = self.BuildPath(self.head,self.tail,ax, plt, start_time)
                return res
